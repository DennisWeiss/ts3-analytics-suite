...
public class DbUserInfoWriter implements Runnable {
    ...
    private void initializeDB() {
        ...
        try {
            statement.execute("CREATE TABLE Users(ClientID INTEGER, " + "UniqueID VARCHAR(40) PRIMARY KEY, " + "nick_name VARCHAR(100), " +
                    "total_upload INTEGER, " + "total_download INTEGER, " + "ip VARCHAR(15), " + "hostname VARCHAR(100), " + "city VARCHAR(100), " +
                    "region VARCHAR(100), " + "country VARCHAR(100), " + "longitude DECIMAL(8, 5), " + "latitude DECIMAL(8, 5), " +
                    "postal VARCHAR(20), " + "org VARCHAR(100));");
        } catch (SQLException e) {
    ...
    private void writeClientIntoDB(Client client) {
        ...
        try {
            statement.execute("INSERT INTO Users(ClientID, UniqueID, nick_name, total_upload, total_download, ip, hostname, " +
                    "city, region, country, longitude, latitude, postal, org) VALUES(" + client.getId() + ", '" + client.getUniqueID() + "', '" +
                    client.getNickname() + "', " + client.getTotalUpload() + ", " + client.getTotalDownload() + ", '" + client.getIp() + "', '" +
                    client.getHostName() + "', '" + client.getCity() + "', '" + client.getRegion() + "', '" + client.getCountry() + "', " +
                    location.getLongitude() + ", " + location.getLatitude() + ", '" + client.getPostalCode() + "', '" + client.getAssociatedOrg() +
                    "');");
        } catch (SQLException e) {
            try {
